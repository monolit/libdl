...
def ensure_directory_exists(directory_path):
    ...
    else: print(f"Directory {directory_path} already exists")

def download(url, path, recreate=False, quiet=False, filename=None, headers={}):
    ensure_directory_exists(path)
    if not filename: filename = url.split('/')[-1]
    filepath = os.path.join(path, filename)
    server_bytes = int(requests.head(url, timeout=7, headers=headers).headers["Content-Length"])
    if not quiet:
                    ...
                    for chunk in r.iter_content(chunk_size=CHUNK_SIZE):
                        ...
                        pbar.set_postfix(
                            {'speed': '{:.2f} B/s'.format(download_speed),
                             'time_left': str(datetime.timedelta(seconds=(server_bytes - downloaded_size) / download_speed))})
                        if len(avg_speeds) == 0 or current_time - AVERAGE_SPEED_WINDOW < avg_speeds[-1][1]:
                            avg_speeds.append((download_speed, current_time))
                        else:
                            while avg_speeds and current_time - AVERAGE_SPEED_WINDOW > avg_speeds[0][1]:
                                avg_speeds.pop(0)
                            avg_speeds.append((download_speed, current_time))
                        if avg_speeds:
                            avg_speed = round(sum(speed for speed, _ in avg_speeds) / len(avg_speeds) / 1024)
                            pbar.set_postfix(
                                {'avg_speed': '{:.2f} KB/s\n'.format(avg_speed)})
...
def pls_run_thrgh(smth=None, **kwargs):
    ...
    while attempts < max_attempts and attempts > 0:
        ...
        except KeyboardInterrupt: sys.exit('exiting, see ya')
